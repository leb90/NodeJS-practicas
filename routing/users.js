...
userModel.getUsers = function(callback)
{
	if (connection) 
	{
		connection.query('SELECT * FROM usuario ORDER BY id', function(error, rows) {
			if(error)
			{
				throw error;
			}
...
userModel.updateUser = function(userData, callback)
{
	...
	if(connection)
	{
		var sql = 'UPDATE usuario SET username = ' + connection.escape(userData.username) + ',' +  
...
userModel.deleteUser = function(id, callback)
{
	if(connection)
	{
		var sqlExists = 'SELECT * FROM usuario WHERE id = ' + connection.escape(id);
